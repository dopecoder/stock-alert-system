...
func (s *SocketClient) Serve(ctx context.Context) {
	...
	for {
		...
		default:
			...
			if s.autoReconnect {
				wg.Add(1)
				go s.checkConnection(ctx, &wg, Restart)
			}
...
func (s *SocketClient) checkConnection(ctx context.Context, wg *sync.WaitGroup, Restart chan bool) {
	defer wg.Done()
